...
export function MacroReviewView({ game = "VALORANT", seriesId }: MacroReviewViewProps) {
  ...
  // Fetch match data - use shared cache for consistency
  useEffect(() => {
    if (!seriesId) {
      setLoading(false);
      return;
    ...
    const fetchData = async () => {
      ...
      if (cached?.matchData && cached.matchData.team1.name !== 'Team 1') {
        console.log('[MacroReviewView] Using cached match data');
        setMatchData({
      ...
      try {
        ...
        if (data.status === 'success' && data.game_state) {
          ...
          // Update shared cache with match data
        ...
        console.error('[MacroReviewView] Fetch error:', err);
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <span className="text-rose-400 font-medium text-sm">{error}</span>
        </div>
      </div>
    );
  }
      ...
      >
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs text-slate-500 font-mono">// TEAM STRATEGY</span>
          </div>
            ...
            <div className="flex bg-black/40 p-1 rounded-xl border border-white/10">
              <button
                ...
                className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${
                  activeTeam === 'team1'
                    ? (game === 'LoL' ? 'bg-[#C89B3C] text-black shadow-lg' : 'bg-[#FF4655] text-white shadow-lg')
